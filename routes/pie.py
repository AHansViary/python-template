...
def generate_pie_chart(instruments):
    ...
    # if it is, increase it to 0.05 and decrease the next one for adjustment
    if any(p < 0.05 for p in proportions):
        for i in range(len(proportions)):
            if proportions[i] < 0.05:
                proportions[i] = 0.05
                proportions[i+1] -= 0.05 - proportions[i]
